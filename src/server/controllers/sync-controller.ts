...
export class SyncController {
  ...
  public async sync(): Promise<ISyncReport> {
    ...
    report.worklogsOutOfTime.push(`Worklog range: ${dateUtils.formatDateTime(startDate)} - ${dateUtils.formatDateTime(endDate)}`);
    let allWorklogList = await this.jiraModel.getLastWorklogs(startDate, endDate, report.worklogsOutOfTime);
    ...
    if (this.projectConfig.jira.nitsCustomFieldIsArtifact) {
      ...
      allWorklogList = this.jiraModel.filterWorklogsByRulesAndAssignWtmConfig(allWorklogList, issuesById, wtmTsConfigPerIssueKey, artifactSettingsList, report);
    }
